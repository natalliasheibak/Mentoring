    ...
    public class StoreManagerController : Controller
    {
        ...
        public async Task<ActionResult> Create(Album album)
        {
            if (ModelState.IsValid)
            {
                ...
                log.Info($"The {album.Title} album was created.");

                return RedirectToAction("Index");
            }

            log.Error($"The {album.Title} album was not created.");
        ...
        public async Task<ActionResult> Edit(Album album)
        {
            if (ModelState.IsValid)
            {
                ...
                log.Info($"The {album.Title} album was edited.");

                return RedirectToAction("Index");
            }

            log.Error($"The {album.Title} album was not edited.");
        ...
        public async Task<ActionResult> Delete(int id = 0)
        {
            ...
            if (album == null)
            {
                log.Error($"{album.Title} was not deleted - the album was not found.");
                return HttpNotFound();
            }
