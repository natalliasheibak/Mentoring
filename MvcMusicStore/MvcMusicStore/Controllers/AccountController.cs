    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    using (var logInCounter = new PerformanceCounter(PerformanceCounterConstants.LogCategoryName, PerformanceCounterConstants.LoginCounter, PerformanceCounterConstants.InstanceName, false))
                    {
                        logInCounter.Increment();
                    }

                    log.Info($"The user {model.UserName} logged in.");
                    return RedirectToLocal(returnUrl);
                }

                ModelState.AddModelError("", "Invalid username or password.");
                log.Warn("The log in failed due to empty userName");
            }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    log.Info($"The {model.UserName} registered successfully.");
                ...
                AddErrors(result);
                log.Warn("The register failed due to some errors.");
            }
        ...
        public ActionResult LogOff()
        {
            ...
            using (var logOutCounter = new PerformanceCounter(PerformanceCounterConstants.LogCategoryName, PerformanceCounterConstants.LogoutCounter, PerformanceCounterConstants.InstanceName, false))
            {
                logOutCounter.Increment();
            }
            log.Info($"The user successully logged out.");
            return RedirectToAction("Index", "Home");
        }
